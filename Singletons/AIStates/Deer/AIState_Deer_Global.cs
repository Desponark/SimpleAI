...
public class AIState_Deer_Global : State<Cognition> {
	...
	public override State<Cognition> Execute(Cognition entity, double delta) {
		UpdateFlightValue(entity, delta);
	...
	private static void UpdateFlightValue(Cognition entity, double delta) {
	...
	private static Dictionary<Node3D, float> EvaluateThreats(Cognition entity) {
		...
		foreach (var body in entity.Perception.VisibleBodies) {
			...
			threat += body.IsInGroup("Wolf") ? 10 : 0;
			threat += body.IsInGroup("Deer") ? -100 : 0;
